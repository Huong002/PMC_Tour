    ...
    if (tour.itineraries?.length > 0) {
      setItinerary(
        tour.itineraries.map((it: any) => {
          ...
          if (it.timeline) {
            try {
              parsedTimeline = typeof it.timeline === 'string' ? JSON.parse(it.timeline) : it.timeline;
            } catch (e) {
          ...
          return {
            ...
            timeline: Array.isArray(parsedTimeline)
              ? parsedTimeline.map((t: any) => ({
                  time: t.time ?? t.Time ?? '',
      ...
      setItinerary([{ day: 1, title: 'Ngày 1 - Khởi hành', description: 'Mô tả hoạt động ngày đầu tiên.', timeline: [] }]);
    }
    setIsInitialized(true);
  }, [tourResponse, isInitialized]);
  ...
  const addItineraryDay = () => {
    ...
    setItinerary((prev) => [
      ...prev,
      {
        ...
        description: 'Mô tả chi tiết các hoạt động.',
        timeline: []
      },
    ]);
